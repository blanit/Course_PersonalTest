    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                //Reinhard
                    //此处预先计算整个场景贴图的亮度均值adapted_lum，此处简化
                    //L(x, y) = 0.27 * R + 0.67 * G + 0.06 * B
                    //adapted_lum = exp(1/n * sum(log(delta+L)))
                    float adapted_lum = 0.18;
                    float MIDDLE_GREY = 0.18;
                    float L_white = 1.5;
                    var_Cubemap *= adapted_lum / MIDDLE_GREY;
                    var_Cubemap = (var_Cubemap * (1.0 + (var_Cubemap / pow(L_white,2.0)))) / (1.0 + var_Cubemap);
