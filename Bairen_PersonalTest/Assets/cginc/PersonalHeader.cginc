...
half3 ExponentFog(half3 Col, half3 posWS){
    //高度雾(指数衰减)
    half fogDen = pow(lerp(1.0, 2.0, _HeightFalloff), (_WorldSpaceCameraPos.y - posWS.y - _HeightBias)) + 0.05;
    //深度雾(线性衰减)
    half fogInt = (distance(posWS.xyz, _WorldSpaceCameraPos.xyz) - _DistBias) / _DistFalloff;
    //太阳光散射
    half vdotl = dot(normalize(_WorldSpaceCameraPos.xyz - posWS.xyz), normalize( - _WorldSpaceLightPos0.xyz));
    vdotl = max(vdotl, 0);
